...
| Step | 代码量 | 阶段 | 核心功能 |
|:---:|:---:|:---:|:---|
| **0** | 50行 | 基础网络 | HTTP Echo、g++ 编译 |
| **1** | 120行 | 基础网络 | 异步 I/O、CMake |
| **2** | 180行 | 基础网络 | HTTP 长连接优化 |
| **3** | 250行 | Agent核心 | 状态机、历史管理 |
| **4** | 320行 | Agent核心 | 执行引擎、并行执行 |
| **5** | 400行 | Agent核心 | 记忆系统、质量门控 |
| **6** | 480行 | Tools系统 | 同步工具、参数校验 |
| **7** | 560行 | Tools系统 | 异步执行、并发限制 |
| **8** | 640行 | Tools系统 | HTTP工具、代码执行 |
| **9** | 720行 | LLM集成 | 多模型接入 |
| **10** | 800行 | LLM集成 | 流式响应、Fallback |
| **11** | 880行 | 企业级 | 数据隔离、连接池 |
| **12** | 960行 | 企业级 | 租户治理、配额 |
| **13** | 1040行 | 连接部署 | Channel集成 |
| **14** | 1100+行 | 连接部署 | JWT、TLS、Docker |
